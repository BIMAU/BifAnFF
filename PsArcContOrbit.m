function [X,par,p]=PsArcContOrbit(gl,X,par0,p,ds,dt,M,Ns,NewtonOpts)
  ...
  for i=1:Ns
   % Y=2*Ycur-Yold;
    ...
    if rem(i,1) == 0 %second argument should be adapted to change plotting interval
      plot([Y(round(N/4):N:NM);Y(round(N/4))],[Y(N/2+round(N/4):N:NM);Y(N/2+round(N/4))],'k', 'LineWidth',2)
      hold on
      pause(0.1)
    end
  end
      xlabel('Re(A(0.25))','fontsize',16)
      ylabel('Im(A(0.25))','fontsize',16)
      axis('square')
  hold off
  figure(2)
  subplot(2,1,1)
  plot(pars,iters)
  subplot(2,1,2)
      plot(pars,real(lambdas1),'k','LineWidth',2)
  hold on
  plot(pars,abs(imag(lambdas1)),'k','LineWidth',2)
  hold off
  figure(3)
  %subplot(2,1,1)
  plot(pars)
  %subplot(2,1,2)
  figure(4)
  plot(pars,ps,'k','LineWidth',2)
  xlabel('\alpha','fontsize',16)
  ylabel('p','fontsize',16) 
  figure(5)
  plot(pars,nrmA,'k','LineWidth',2);
  xlabel('\alpha','fontsize',16)
